...
ATTESTATION_ID_RE = re.compile(r"/attestations/(\d+)/bundle$")
BULK_BATCH_SIZE = 100
...
def parse_attestation_id(bundle_url: str) -> int | None:
    match = ATTESTATION_ID_RE.search(bundle_url)
    if not match:
        return None
...
def select_deletions(
    ...
    *,
...
) -> tuple[list[AttestationRecord], AttestationRecord | None]:
    if not records:
        return [], None
    ...
    newest = records[0]
    to_delete = [record for record in records[1:] if record.created_at < cutoff]
    return to_delete, newest
...
def format_attestation_row(record: dict[str, object] | None) -> str:
    if not record:
        return "| — | — | — |"
...
def format_summary_markdown(report: dict[str, object]) -> str:
    ...
    lines = [
        ...
        "| --- | --- | --- |",
        format_attestation_row(report["kept"]),
        "",
    ]
...
def main() -> int:
    ...
    digests = collect_data_yml_digests(args.repo_root)
    print(f"Collected {len(digests)} unique data.yml digest(s) from git history.")
    ...
    to_delete, newest = select_deletions(records, max_age_days=args.max_age_days)

    if newest:
        print(
            "Newest attestation "
            f"{newest.attestation_id} from {newest.created_at.isoformat()} "
            f"({newest.subject_digest}) will be kept."
        )
    ...
    report = {
        ...
        "kept": serialize_record(newest) if newest else None,
