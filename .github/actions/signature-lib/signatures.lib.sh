...
_submission_assemble_entry_from_proposals() {
  ...
  jq -n --arg package "$package" --argjson signature "$assembled" \
    '{package: $package, signature: $signature}' | yq -P > "$entry_file"
}
...
#   CUSTOM_FDROID_APK_SHA256, GPLAY_SIG, GPLAY_APK_SHA256, APPVERIFIER_ALL_SIGS, DIRECT_SIG, DIRECT_APK_URL,
#   DIRECT_APK_SHA256
...
submission_build_entry_file() {
  ...
  local apk_sha apk_link apk_repo av_fp
  ...
  if [[ -n "${APPVERIFIER_ALL_SIGS:-}" ]]; then
    while IFS= read -r av_fp; do
      [[ -z "$av_fp" ]] && continue
      fp_block="$(signatures_format_block "$av_fp")"
      _submission_add_source "$fp_block" "AppVerifier"
    done < <(signatures_to_lines "$APPVERIFIER_ALL_SIGS")
  fi
...
submission_merge_entry_into_data_yml() {
  ...
  if [[ -f "$data_file" ]] && [[ -s "$data_file" ]]; then
    ...
    if yq -e '.packages[] | select(.package == strenv(PACKAGE))' "$data_file" >/dev/null 2>&1; then
      ...
      merged_file="$(mktemp)"
      printf '%s' "$merged_json" > "$merged_file"
